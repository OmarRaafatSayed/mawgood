...
<?php $__env->startSection('page-icon', '<i class="fas fa-shopping-cart me-2"></i>'); ?>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="mb-0" id="pending-orders"><?php echo e($stats['orders']['pending'] ?? 0); ?></h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="mb-0" id="processing-orders"><?php echo e($stats['orders']['unshipped'] ?? 0); ?></h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="mb-0" id="shipped-orders"><?php echo e($stats['orders']['shipped'] ?? 0); ?></h2>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <h2 class="mb-0" id="total-orders"><?php echo e($stats['orders']['total'] ?? 0); ?></h2>
    ...
    <div class="card-body">
        <form method="GET" class="row g-3">
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" name="search" value="<?php echo e(request('search')); ?>" placeholder="رقم الطلب أو اسم العميل">
            </div>
            <div class="col-md-2">
                ...
                <select class="form-select" name="status">
                    ...
                    <option value="shipped" <?php echo e(request('status') == 'shipped' ? 'selected' : ''); ?>>مشحون</option>
            ...
            <div class="col-md-3">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="<?php echo e(route('vendor.orders.index')); ?>" class="btn btn-outline-secondary">
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center gap-3">
            <small class="text-muted">عرض <?php echo e($orders->firstItem() ?? 0); ?> - <?php echo e($orders->lastItem() ?? 0); ?> من <?php echo e($orders->total() ?? 0); ?></small>
        </div>
    </div>
            ...
            <table class="table table-hover mb-0" id="orders-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <strong>#<?php echo e($order->increment_id ?? $order->id); ?></strong>
                            <br><small class="text-muted"><?php echo e($order->created_at ? $order->created_at->format('H:i') : ''); ?></small>
                        </td>
                        <td>
                            <div>
                                <h6 class="mb-1"><?php echo e($order->customer_name ?? 'عميل'); ?></h6>
                                <small class="text-muted"><?php echo e($order->customer_email ?? ''); ?></small>
                            </div>
                        </td>
                        <td>
                            <span class="badge bg-info"><?php echo e($order->items_count ?? 0); ?> منتج</span>
                        </td>
                        <td>
                            <strong><?php echo e(number_format($order->total_amount ?? 0, 2)); ?> جنيه</strong>
                        </td>
                        <td>
                            <?php
                            ...
                            ?>
                            <span class="badge bg-<?php echo e($config['class']); ?>"><?php echo e($config['text']); ?></span>
                        </td>
                        <td><?php echo e($order->created_at ? $order->created_at->format('Y-m-d') : 'N/A'); ?></td>
                        <td>
                            <div class="btn-group" role="group">
                                <a href="<?php echo e(route('vendor.orders.show', $order->id)); ?>" class="btn btn-sm btn-outline-info" title="عرض التفاصيل">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if($status == 'pending'): ?>
                                <button type="button" class="btn btn-sm btn-outline-success" onclick="updateOrderStatus(<?php echo e($order->id); ?>, 'processing')" title="قبول الطلب">
                                    <i class="fas fa-check"></i>
                                </button>
                                <?php endif; ?>
                                <?php if($status == 'processing'): ?>
                                <button type="button" class="btn btn-sm btn-outline-primary" onclick="updateOrderStatus(<?php echo e($order->id); ?>, 'shipped')" title="شحن الطلب">
                                ...
                                <div class="btn-group" role="group">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="<?php echo e(route('vendor.orders.invoice', $order->id)); ?>">
                                            <i class="fas fa-file-invoice me-2"></i>طباعة الفاتورة
                                        </a></li>
                                        <li><a class="dropdown-item" href="<?php echo e(route('vendor.orders.shipping_label', $order->id)); ?>">
                                            <i class="fas fa-shipping-fast me-2"></i>ملصق الشحن
                                        </a></li>
                                        <li><hr class="dropdown-divider"></li>
                                        <li><a class="dropdown-item text-danger" href="#" onclick="cancelOrder(<?php echo e($order->id); ?>)">
...
function confirmStatusUpdate() {
    ...
    fetch(`<?php echo e(route("vendor.orders.index")); ?>/${formData.get('order_id')}/status`, {
        method: 'PATCH',
        headers: {
...
function executeBulkOrderAction() {
    ...
    fetch('<?php echo e(route("vendor.orders.index")); ?>/bulk-action', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function exportOrders() {
    ...
    window.location.href = `<?php echo e(route("vendor.orders.index")); ?>?${params.toString()}`;
}
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('vendor.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\EXPRESS\Downloads\coding\mawgood\mawgood\packages\Mawgood\Vendor\src\Providers/../Resources/views/orders/index.blade.php ENDPATH**/ ?>