<div id="searchModal" class="search-modal">
    <div class="search-content">
        <div class="search-header">
            ...
            <form action="{{ route('shop.search.index') }}" method="GET" class="search-form">
                <input type="text" name="query" placeholder="@lang('shop::app.components.layouts.header.desktop.bottom.search-text')" autocomplete="off" autofocus>
                <button type="submit">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="search-body">
            @if(auth()->check() && auth()->user()->search_history)
            <div class="search-section">
                <div class="section-header">
                    <h4>@lang('shop::app.search.title')</h4>
                    <button type="button">@lang('shop::app.categories.filters.clear-all')</button>
                </div>
                <div class="search-items">
                    @foreach(json_decode(auth()->user()->search_history, true) ?? [] as $term)
                    <a href="{{ route('shop.search.index', ['query' => $term]) }}" class="search-item">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="search-section">
                <div class="section-header">
                    <h4>@lang('shop::app.search.results')</h4>
                </div>
                <div class="search-tags">
                    @php $tags = ['Electronics', 'Fashion', 'Home', 'Beauty', 'Sports', 'Books']; @endphp
                    @foreach($tags as $tag)
                    <a href="{{ route('shop.search.index', ['query' => $tag]) }}" class="tag">{{ $tag }}</a>
                    @endforeach
                </div>
            </div>
        </div>
